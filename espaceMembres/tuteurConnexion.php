...
if (isset($_POST['btnConnexion'])) {
	...
	if (!empty($loginConnect) AND !empty($mdpConnect)) {
		$userEmail = $bdd->prepare('SELECT * FROM tuteurs WHERE emailTuteur = ? AND password = ?');
		$userEmail->execute(array($loginConnect, $mdpConnect));
		$userExist = $userEmail->rowCount();
		if ($userExist == 1) {
			$userinfo = $userEmail->fetch();
			$_SESSION['idTuteur'] = $userinfo['idTuteur'];
			$_SESSION['nameTuteur'] = $userinfo['nameTuteur'];
			$_SESSION['emailTuteur'] = $userinfo['emailTuteur'];
			$_SESSION['password'] = $userinfo['password'];
			header('location:tuteurProfil.php?id='.$_SESSION['idTuteur']);
		}else{
			header('location:tuteurConnexion.php?erreur=Ce compte n\'éxiste pas !');
		}
	}else{
		header('location:tuteurConnexion.php?erreur=Tous les champs doivent être remplis !');
	}
}
