	...
	if (!empty($pseudo) AND !empty($email) AND !empty($email2) AND !empty($mdp) AND !empty($mdp2)) {
		...
		if ($pseudolength <= 255) {
			if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
				if ($email == $email2) {
					$emailexist = $bdd->prepare('SELECT * FROM pseudo WHERE email = ?');
					$emailexist->execute(array($email));
					$isExist = $emailexist->rowCount();
					// Vérification des doublons 
					if ($isExist == 0) {
						if ($mdp == $mdp2) {
							$insertMembre = $bdd->prepare('INSERT INTO pseudo(nom, email, password) VALUES (?, ?, ?)');
							$insertMembre->execute(array($pseudo, $email, $mdp));
