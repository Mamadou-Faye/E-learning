...
if (isset($_POST['btnSubmit'])) {
	...
	if (!empty($pseudo) AND !empty($email) AND !empty($email2) AND !empty($numAdmin) AND !empty($mdp) AND !empty($mdp2)) {
		$pseudolength = strlen($pseudo);
		if ($pseudolength <= 255) {
			if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
				if ($email == $email2) {
					$emailexist = $bdd->prepare('SELECT * FROM admin WHERE email = ?');
					$emailexist->execute(array($email));
					$isExist = $emailexist->rowCount();
					// Vérification des doublons 
					if ($isExist == 0) {
						if ($mdp == $mdp2) {
							$insertMembre = $bdd->prepare('INSERT INTO admin(name, email, numAdmin, password) VALUES (?, ?, ?, ?)');
							$insertMembre->execute(array($pseudo, $email, $numAdmin, $mdp));
							header('location:inscription.php?msg=Votre compte à été bien créé ! <a href="connexion.php">Se connecter</a>');
						}else{
							header('location:inscription.php?erreur=Vos mots de passe ne correspondent pas !');
						}
					}else{
						header('location:inscription.php?erreur=Cette adresse email existe déjà !');
					}
				}else{
					header('location:inscription.php?erreur=Vos adresses emails ne correspondent pas !');
				}
			}else{
				header('location:inscription.php?erreur=Votre email n\'est pas valide !');
			}
		}else{
			header('location:inscription.php?erreur=Votre pseudo ne doit pas dépasser 255 caractères !');
		}
	}else {
		header('location:inscription.php?erreur=Veuillez remplir tous les champs svp !');
	}
}
					...
					<div class="section_title text-center">
						<div align="center">
							...
							<form style="width: 60%;" action="" method="post">
								...
								<div class="input-group">
									<input type="number" name="numberAdmin" class="form-control" placeholder="Votre numéro">
								</div><br/>
								<div class="input-group">
