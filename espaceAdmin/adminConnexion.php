...
if (isset($_POST['btnConnexion'])) {
	...
	if (!empty($loginConnect) AND !empty($mdpConnect)) {
		$emailuser = $bdd->prepare('SELECT * FROM admin WHERE email = ? AND password = ?');
		...
		if ($emailexist == 1) {
			...
			header('location:adminConnexion.php?erreur=Ce compte n\'existe pas !');
		}
	}else{
		header('location:adminConnexion.php?erreur=Veuillez remplir tous les champs svp !');
	}
}
?>
<!DOCTYPE html>
